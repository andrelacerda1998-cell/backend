...
/**
 * A cobrança de um extra aprovado falhou — o técnico fica a saber que aquele
 * valor NÃO vai entrar no pagamento do serviço.
 */
class ServiceExtraChargeFailedNotification extends Notification implements ShouldQueue
{
    ...
    {
        return __('notifications.serviceExtra.chargeFailed.vendor.title', [], $language);
    }

    private function body($language): string
    {
        return __('notifications.serviceExtra.chargeFailed.vendor.description', [], $language).$this->itemLabel($language);
    }

...
/**
 * Os 308 concelhos de Portugal para a sondagem de zonas.
 *
 * Entram em `survey_cities` como sondagem — NÃO como zonas ativas. Zona ativa
 * (allowed_zones) significa "a Piquet já opera aqui" e é decisão de negócio;
 * um seeder não pode ligar operação numa cidade. Aqui o técnico apenas vota
 * para a Piquet abrir onde ele está.
 *
 * Idempotente: updateOrCreate por (city, district). Correr vezes seguidas não
 * duplica. Concelhos que já sejam allowed_zone são saltados, para não aparecer
 * a mesma cidade como "Ativa" e "Em breve" ao mesmo tempo.
 */
class PortugueseMunicipalitiesSeeder extends Seeder
{
    public function run(): void
    {
        $active = AllowedZone::query()
            ->get()
            ->map(fn ($z) => mb_strtolower(trim($z->city)).'|'.mb_strtolower(trim((string) $z->district)))
            ->all();

        $inserted = 0;
        $skipped = 0;

        foreach ($this->municipalities() as $district => $cities) {
        ...
        $this->command?->info("Concelhos processados: {$inserted} · saltados (já ativos): {$skipped}");
    }

    /** @return array<string, string[]> distrito => concelhos */
